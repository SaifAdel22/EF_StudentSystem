// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using StudentSystem.Data;

#nullable disable

namespace StudentSystem.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20240919200321_License")]
    partial class License
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("StudentSystem.Entities.Course", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .HasMaxLength(50)
                        .HasColumnType("VARCHAR");

                    b.Property<DateTime>("EndDate")
                        .HasColumnType("date");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("VARCHAR");

                    b.Property<decimal>("Price")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("date");

                    b.HasKey("Id");

                    b.ToTable("Courses", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Description = "Basic Math Course",
                            EndDate = new DateTime(2025, 1, 31, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Name = "Mathematics",
                            Price = 200.00m,
                            StartDate = new DateTime(2024, 8, 1, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 2,
                            Description = "Fundamentals of Physics",
                            EndDate = new DateTime(2025, 2, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Name = "Physics",
                            Price = 250.00m,
                            StartDate = new DateTime(2024, 9, 5, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 3,
                            Description = "Introduction to Programming",
                            EndDate = new DateTime(2025, 2, 28, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Name = "Computer Science",
                            Price = 300.00m,
                            StartDate = new DateTime(2024, 10, 1, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        });
                });

            modelBuilder.Entity("StudentSystem.Entities.HomeWork", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("VARCHAR");

                    b.Property<int>("ContentType")
                        .HasMaxLength(50)
                        .HasColumnType("int");

                    b.Property<int>("CourseId")
                        .HasColumnType("int");

                    b.Property<int>("StudentId")
                        .HasColumnType("int");

                    b.Property<DateTime>("SubmissionDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("CourseId");

                    b.HasIndex("StudentId");

                    b.ToTable("HomeWorks", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Content = "Math Homework 1",
                            ContentType = 1,
                            CourseId = 1,
                            StudentId = 1,
                            SubmissionDate = new DateTime(2024, 10, 15, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 2,
                            Content = "Physics Homework 1",
                            ContentType = 1,
                            CourseId = 2,
                            StudentId = 2,
                            SubmissionDate = new DateTime(2024, 10, 20, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 3,
                            Content = "Programming Homework 1",
                            ContentType = 2,
                            CourseId = 3,
                            StudentId = 3,
                            SubmissionDate = new DateTime(2024, 10, 25, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        });
                });

            modelBuilder.Entity("StudentSystem.Entities.License", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("VARCHAR");

                    b.Property<int>("ResourceId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ResourceId");

                    b.ToTable("Licenses", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Basic License",
                            ResourceId = 1
                        },
                        new
                        {
                            Id = 2,
                            Name = "Pro License",
                            ResourceId = 1
                        },
                        new
                        {
                            Id = 3,
                            Name = "Student License",
                            ResourceId = 1
                        },
                        new
                        {
                            Id = 4,
                            Name = "Enterprise License",
                            ResourceId = 2
                        },
                        new
                        {
                            Id = 5,
                            Name = "Basic License",
                            ResourceId = 2
                        },
                        new
                        {
                            Id = 6,
                            Name = "Student License",
                            ResourceId = 3
                        });
                });

            modelBuilder.Entity("StudentSystem.Entities.Resource", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int");

                    b.Property<int>("CourseId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("VARCHAR");

                    b.Property<int>("ResourceType")
                        .HasColumnType("int");

                    b.Property<string>("URL")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CourseId");

                    b.ToTable("Resources", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CourseId = 1,
                            Name = "Math Resource 1",
                            ResourceType = 0,
                            URL = "http://example.com/math1"
                        },
                        new
                        {
                            Id = 2,
                            CourseId = 1,
                            Name = "Math Resource 2",
                            ResourceType = 0,
                            URL = "http://example.com/math2"
                        },
                        new
                        {
                            Id = 3,
                            CourseId = 1,
                            Name = "Math Resource 3",
                            ResourceType = 0,
                            URL = "http://example.com/math3"
                        },
                        new
                        {
                            Id = 4,
                            CourseId = 1,
                            Name = "Math Resource 4",
                            ResourceType = 1,
                            URL = "http://example.com/math4"
                        },
                        new
                        {
                            Id = 5,
                            CourseId = 1,
                            Name = "Math Resource 5",
                            ResourceType = 2,
                            URL = "http://example.com/math5"
                        },
                        new
                        {
                            Id = 6,
                            CourseId = 1,
                            Name = "Math Resource 6",
                            ResourceType = 3,
                            URL = "http://example.com/math6"
                        },
                        new
                        {
                            Id = 7,
                            CourseId = 2,
                            Name = "Physics Resource 1",
                            ResourceType = 0,
                            URL = "http://example.com/physics1"
                        },
                        new
                        {
                            Id = 8,
                            CourseId = 2,
                            Name = "Physics Resource 2",
                            ResourceType = 2,
                            URL = "http://example.com/physics2"
                        },
                        new
                        {
                            Id = 9,
                            CourseId = 2,
                            Name = "Physics Resource 3",
                            ResourceType = 3,
                            URL = "http://example.com/physics3"
                        },
                        new
                        {
                            Id = 10,
                            CourseId = 3,
                            Name = "CS Resource 1",
                            ResourceType = 0,
                            URL = "http://example.com/cs1"
                        },
                        new
                        {
                            Id = 11,
                            CourseId = 3,
                            Name = "CS Resource 2",
                            ResourceType = 0,
                            URL = "http://example.com/cs2"
                        },
                        new
                        {
                            Id = 12,
                            CourseId = 3,
                            Name = "CS Resource 3",
                            ResourceType = 0,
                            URL = "http://example.com/cs3"
                        },
                        new
                        {
                            Id = 13,
                            CourseId = 3,
                            Name = "CS Resource 4",
                            ResourceType = 0,
                            URL = "http://example.com/cs4"
                        },
                        new
                        {
                            Id = 14,
                            CourseId = 3,
                            Name = "CS Resource 5",
                            ResourceType = 0,
                            URL = "http://example.com/cs5"
                        },
                        new
                        {
                            Id = 15,
                            CourseId = 3,
                            Name = "CS Resource 6",
                            ResourceType = 0,
                            URL = "http://example.com/cs6"
                        },
                        new
                        {
                            Id = 16,
                            CourseId = 3,
                            Name = "CS Resource 7",
                            ResourceType = 0,
                            URL = "http://example.com/cs7"
                        });
                });

            modelBuilder.Entity("StudentSystem.Entities.Student", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int");

                    b.Property<DateTime?>("BirthDay")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("VARCHAR");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("Regeistration")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.ToTable("Students", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            BirthDay = new DateTime(2000, 2, 15, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Name = "John Doe",
                            PhoneNumber = "123-456-7890",
                            Regeistration = new DateTime(2024, 9, 15, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 2,
                            BirthDay = new DateTime(1999, 3, 10, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Name = "Jane Smith",
                            PhoneNumber = "987-654-3210",
                            Regeistration = new DateTime(2024, 9, 15, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        },
                        new
                        {
                            Id = 3,
                            Name = "Alex Johnson",
                            Regeistration = new DateTime(2024, 9, 15, 0, 0, 0, 0, DateTimeKind.Unspecified)
                        });
                });

            modelBuilder.Entity("StudentSystem.Entities.StudentCourses", b =>
                {
                    b.Property<int>("CourseId")
                        .HasColumnType("int");

                    b.Property<int>("StudentId")
                        .HasColumnType("int");

                    b.HasKey("CourseId", "StudentId");

                    b.HasIndex("StudentId");

                    b.ToTable("StudentCourses", (string)null);

                    b.HasData(
                        new
                        {
                            CourseId = 1,
                            StudentId = 1
                        },
                        new
                        {
                            CourseId = 3,
                            StudentId = 1
                        },
                        new
                        {
                            CourseId = 2,
                            StudentId = 2
                        },
                        new
                        {
                            CourseId = 3,
                            StudentId = 3
                        });
                });

            modelBuilder.Entity("StudentSystem.Entities.HomeWork", b =>
                {
                    b.HasOne("StudentSystem.Entities.Course", "Course")
                        .WithMany("HomeWorks")
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentSystem.Entities.Student", "Student")
                        .WithMany("HomeWorks")
                        .HasForeignKey("StudentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Course");

                    b.Navigation("Student");
                });

            modelBuilder.Entity("StudentSystem.Entities.License", b =>
                {
                    b.HasOne("StudentSystem.Entities.Resource", "Resource")
                        .WithMany("Licenses")
                        .HasForeignKey("ResourceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Resource");
                });

            modelBuilder.Entity("StudentSystem.Entities.Resource", b =>
                {
                    b.HasOne("StudentSystem.Entities.Course", "Course")
                        .WithMany("Resources")
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Course");
                });

            modelBuilder.Entity("StudentSystem.Entities.StudentCourses", b =>
                {
                    b.HasOne("StudentSystem.Entities.Course", "Course")
                        .WithMany("StudentCourses")
                        .HasForeignKey("CourseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentSystem.Entities.Student", "Student")
                        .WithMany("StudentCourses")
                        .HasForeignKey("StudentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Course");

                    b.Navigation("Student");
                });

            modelBuilder.Entity("StudentSystem.Entities.Course", b =>
                {
                    b.Navigation("HomeWorks");

                    b.Navigation("Resources");

                    b.Navigation("StudentCourses");
                });

            modelBuilder.Entity("StudentSystem.Entities.Resource", b =>
                {
                    b.Navigation("Licenses");
                });

            modelBuilder.Entity("StudentSystem.Entities.Student", b =>
                {
                    b.Navigation("HomeWorks");

                    b.Navigation("StudentCourses");
                });
#pragma warning restore 612, 618
        }
    }
}
